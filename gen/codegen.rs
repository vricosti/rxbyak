...
const HANDWRITTEN: &[&str] = &[
    ...
    "vpand",
    "vpor",
    // VEX vector-result and EVEX opmask-result overloads share one Rust
    // dispatcher for each base compare mnemonic.
    "vcmppd",
    "vcmpps",
    "vcmpsd",
    "vcmpss",
...
// Source: Xbyak gen/gen_code.cpp `vcmpeqps` table. The array index is the
// architectural compare predicate immediate.
const COMPARE_PREDICATES: [&str; 32] = [
    "eq", "lt", "le", "unord", "neq", "nlt", "nle", "ord", "eq_uq", "nge", "ngt", "false",
    "neq_oq", "ge", "gt", "true", "eq_os", "lt_oq", "le_oq", "unord_s", "neq_us", "nlt_uq",
    "nle_uq", "ord_s", "eq_us", "nge_uq", "ngt_uq", "false_os", "neq_os", "ge_oq", "gt_oq",
    "true_us",
];

// Source: Xbyak gen/gen_code.cpp PCLMULQDQ alias table.
const PCLMUL_ALIASES: [(&str, u8); 4] = [
    ("pclmullqlqdq", 0x00),
    ("pclmulhqlqdq", 0x01),
    ("pclmullqhqdq", 0x10),
    ("pclmulhqhqdq", 0x11),
...
const RUST_KEYWORDS: &[&str] = &[
    "and", "or", "not", "xor", "in", "loop", "mod", "ref", "type", "yield",
];
...
pub fn generate<W: Write>(out: &mut W, tables: &[&[Insn]]) -> std::io::Result<()> {
        ...
        for insn in *table {
            ...
            match insn.pattern {
                Pattern::Sse => {
                    ...
                    writeln!(
                        out,
                        "        self.buf.op_sse(&dst, &src.into(), {}, 0x{:02X}, None)",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::SseImm => {
                    ...
                    writeln!(
                        out,
                        "        self.buf.op_sse(&dst, &src.into(), {}, 0x{:02X}, Some(imm))",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::AvxXXXm => {
                    writeln!(
                        out,
                        "    /// `{} xmm/ymm/zmm, xmm/ymm/zmm, xmm/ymm/zmm/m`",
                        insn.name
                    )?;
                    writeln!(out, "    pub fn {}(&mut self, x1: Reg, x2: Reg, op: impl Into<RegMem>) -> Result<()> {{", name)?;
                    writeln!(
                        out,
                        "        self.op_avx_x_x_xm(x1, x2, op, {}, 0x{:02X}, None)",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::AvxXXXmImm => {
                    writeln!(
                        out,
                        "    /// `{} xmm/ymm/zmm, xmm/ymm/zmm, xmm/ymm/zmm/m, imm8`",
                        insn.name
                    )?;
                    writeln!(out, "    pub fn {}(&mut self, x1: Reg, x2: Reg, op: impl Into<RegMem>, imm: u8) -> Result<()> {{", name)?;
                    writeln!(
                        out,
                        "        self.op_avx_x_x_xm(x1, x2, op, {}, 0x{:02X}, Some(imm))",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::AvxKXXm => {
                    ...
                    writeln!(
                        out,
                        "        self.op_avx_k_x_xm(k, x2, op, {}, 0x{:02X}, None)",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                ...
                Pattern::VexXXm => {
                    ...
                    writeln!(
                        out,
                        "        self.buf.op_vex(&dst, None, &src.into(), {}, 0x{:02X}, None)",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::VexXXmImm => {
                    writeln!(
                        out,
                        "    /// `{} xmm/ymm/zmm, xmm/ymm/zmm/m, imm8`",
                        insn.name
                    )?;
                    writeln!(out, "    pub fn {}(&mut self, dst: Reg, src: impl Into<RegMem>, imm: u8) -> Result<()> {{", name)?;
                    writeln!(
                        out,
                        "        self.buf.op_vex(&dst, None, &src.into(), {}, 0x{:02X}, Some(imm))",
                        flags, opcode
                    )?;
                    writeln!(out, "    }}")?;
                }
                Pattern::VexMov => {
                    ...
                    writeln!(
                        out,
                        "    /// `{} xmm/ymm/zmm, xmm/ymm/zmm/m` or `{0} m, xmm/ymm/zmm`",
                        insn.name
                    )?;
                    ...
                    writeln!(
                        out,
                        "                self.buf.op_vex(d, None, &src, type_, 0x{:02X}, None)",
                        opcode
                    )?;
                    writeln!(out, "            }}")?;
                    ...
                    writeln!(
                        out,
                        "                self.buf.op_vex(s, None, &dst, type_, 0x{:02X}, None)",
                        store_opcode
                    )?;
                    writeln!(out, "            }}")?;
                    writeln!(
                        out,
                        "            _ => Err(crate::error::Error::BadCombination),"
                    )?;
                    writeln!(out, "        }}")?;
                    writeln!(out, "    }}")?;
                }
    ...
    // Xbyak generates these named compare predicates as thin wrappers around
    // CMP{P,S}{D,S} / VCMP{P,S}{D,S} with the table index as imm8.
    for suffix in ["pd", "ps", "sd", "ss"] {
        for (predicate_imm, predicate_name) in COMPARE_PREDICATES.iter().enumerate() {
            if predicate_imm < 8 {
                let name = format!("cmp{}{}", predicate_name, suffix);
                let base = if suffix == "sd" {
                    "cmpsd_xmm".to_string()
                } else {
                    format!("cmp{}", suffix)
                };
                writeln!(out, "    /// Xbyak `{}` compare-predicate alias.", name)?;
                writeln!(
                    out,
                    "    pub fn {}(&mut self, dst: Reg, src: impl Into<RegMem>) -> Result<()> {{",
                    name
                )?;
                writeln!(out, "        self.{}(dst, src, {})", base, predicate_imm)?;
                writeln!(out, "    }}")?;
                writeln!(out)?;
                generated_count += 1;
            }

            let name = format!("vcmp{}{}", predicate_name, suffix);
            let base = format!("vcmp{}", suffix);
            writeln!(out, "    /// Xbyak `{}` compare-predicate alias.", name)?;
            writeln!(
                out,
                "    pub fn {}(&mut self, dst: Reg, src1: Reg, src2: impl Into<RegMem>) -> Result<()> {{",
                name
            )?;
            writeln!(
                out,
    ...
    for (name, imm) in PCLMUL_ALIASES {
        writeln!(out, "    /// Xbyak `{}` PCLMULQDQ alias.", name)?;
        writeln!(
            out,
            "    pub fn {}(&mut self, dst: Reg, src: impl Into<RegMem>) -> Result<()> {{",
            name
        )?;
        writeln!(out, "        self.pclmulqdq(dst, src, 0x{:02X})", imm)?;
        writeln!(out, "    }}")?;
        writeln!(out)?;
        generated_count += 1;

        let vector_name = format!("v{}", name);
        writeln!(out, "    /// Xbyak `{}` VPCLMULQDQ alias.", vector_name)?;
        writeln!(
            out,
            "    pub fn {}(&mut self, dst: Reg, src1: Reg, src2: impl Into<RegMem>) -> Result<()> {{",
            vector_name
        ...
        writeln!(
            out,
            "        self.vpclmulqdq(dst, src1, src2, 0x{:02X})",
            imm
        )?;
        writeln!(out, "    }}")?;
        writeln!(out)?;
    ...
    writeln!(
        out,
        "// Generated {} methods ({} skipped as hand-written, {} skipped as duplicate)",

...
fn test_nasm_mov_reg32_reg32() {
    ...
    for &(dst, dst_name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &(src, src_name) in REGS32.iter().chain(REGS32_EXT.iter()) {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(dst, src)),
            ));
        }
    }
...
fn test_nasm_mov_reg32_imm() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
        ...
        fn $test_name() {
            ...
            for &(dst, dst_name) in $regs {
                for &(src, src_name) in $regs {
                    ...
                    insns.push((
                        asm_text,
                        Box::new(move |a: &mut CodeAssembler| a.$method(dst, src)),
                    ));
                }
            }
            compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_add_reg32_imm() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.add(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_sub_reg64_imm() {
    ...
    for &(reg, name) in REGS64.iter().chain(REGS64_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.sub(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_cmp_reg32_imm() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.cmp(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_and_reg64_imm() {
    ...
    for &(reg, name) in REGS64.iter().chain(REGS64_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.and_(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_shl_reg32() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in shift_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.shl(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_shr_reg64() {
    ...
    for &(reg, name) in REGS64.iter().chain(REGS64_EXT.iter()) {
        for &imm in shift_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.shr(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_sar_reg32() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in shift_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.sar(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_push_imm() {
    ...
    for &imm in imm_values {
        ...
        insns.push((
            asm_text,
            Box::new(move |a: &mut CodeAssembler| a.push_imm(imm)),
        ));
    }
    compare_nasm_batch(&nasm, 64, insns);
}
...
fn test_nasm_test_reg32_reg32() {
    ...
    for &(dst, dst_name) in regs {
        for &(src, src_name) in regs {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.test(dst, src)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_test_reg32_imm() {
    ...
    for &(reg, name) in REGS32.iter().chain(REGS32_EXT.iter()) {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.test(reg, imm)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_xchg_reg32() {
    ...
    for &(dst, dst_name) in regs {
        for &(src, src_name) in regs {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.xchg(dst, src)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_imul_reg32_reg32() {
    ...
    for &(dst, dst_name) in regs {
        for &(src, src_name) in regs {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.imul(dst, src)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_lea_variants() {
    ...
    for &(base, base_name) in bases {
        for &disp in disps {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.lea(RAX, ptr(base + disp))),
            ));
        }
    }

    // lea reg, [base + index*scale + disp]
    let indices: &[(Reg, &str)] = &[(RCX, "rcx"), (RDX, "rdx"), (RSI, "rsi"), (R8, "r8")];
    for &(base, base_name) in &[(RAX, "rax"), (RBX, "rbx"), (R8, "r8")] {
        for &(idx, idx_name) in indices {
            for &scale in &[1u8, 2, 4, 8] {
                let asm_text = format!("lea rax, [{}+{}*{}+0x10]", base_name, idx_name, scale);
                insns.push((
                    asm_text,
...
fn test_nasm_mov_reg_mem() {
    ...
    for &(reg, reg_name) in regs {
        for &(base, base_name) in bases {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(reg, dword_ptr(base.into()))),
            ));
        }
    }
    ...
    for &(reg, reg_name) in regs {
        for &(base, base_name) in bases {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(dword_ptr(base.into()), reg)),
            ));
        }
    }
    ...
    for &(reg, reg_name) in regs {
        for &(base, base_name) in bases {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(reg, dword_ptr(base + 0x10))),
            ));
        }
    }
...
fn test_nasm_mov_reg64_mem() {
    ...
    for &(reg, reg_name) in regs {
        for &(base, base_name) in bases {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(reg, qword_ptr(base.into()))),
            ));
        }
    }
    ...
    for &(reg, reg_name) in regs {
        for &(base, base_name) in bases {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.mov(qword_ptr(base.into()), reg)),
            ));
        }
    }
...
fn test_nasm_adc_sbb_reg32() {
    ...
    for &(dst, dst_name) in regs {
        for &(src, src_name) in regs {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.adc(dst, src)),
            ));
        }
    }
    for &(dst, dst_name) in regs {
        for &(src, src_name) in regs {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.sbb(dst, src)),
            ));
        }
    }
    compare_nasm_batch(&nasm, 64, insns);
...
fn test_nasm_mov_indexed_mem() {
    ...
    for &(base, base_name, idx, idx_name, scale, disp) in combos {
        ...
        insns.push((
            asm_text,
            Box::new(move |a: &mut CodeAssembler| a.mov(EAX, dword_ptr(base + idx * scale + disp))),
        ));
    }
...
fn test_nasm_ret_imm() {
    ...
    let insns: Vec<(String, Box<dyn FnOnce(&mut CodeAssembler) -> Result<()>>)> = vec![
        ...
        (
            "ret 256".into(),
            Box::new(|a: &mut CodeAssembler| a.ret_imm(256)),
        ),
    ];
    compare_nasm_batch(&nasm, 64, insns);
}
...
fn test_nasm_or_and_xor_imm() {
    ...
    for &(reg, name) in regs {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.or_(reg, imm)),
            ));
        }
    }
    for &(reg, name) in regs {
        for &imm in imm_values {
            ...
            insns.push((
                asm_text,
                Box::new(move |a: &mut CodeAssembler| a.xor_(reg, imm)),
            ));
        }
    }
...
fn test_nasm_add_sub_mem() {
    ...
    for &(reg, reg_name, base, base_name) in combos {
        ...
        insns.push((
            asm_text,
            Box::new(move |a: &mut CodeAssembler| a.add(reg, dword_ptr(base.into()))),
        ));
    }
    ...
    for &(reg, reg_name, base, base_name) in combos {
        ...
        insns.push((
            asm_text,
            Box::new(move |a: &mut CodeAssembler| a.sub(reg, dword_ptr(base.into()))),
        ));
    }
    ...
    for &(reg, reg_name, base, base_name) in combos {
        ...
        insns.push((
            asm_text,
            Box::new(move |a: &mut CodeAssembler| a.add(dword_ptr(base.into()), reg)),
        ));
    }

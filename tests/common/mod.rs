...
fn temp_dir() -> PathBuf {
    let dir =
        PathBuf::from(std::env::var("CARGO_TARGET_TMPDIR").unwrap_or_else(|_| "/tmp".to_string()))
            .join("nasm_tmp");
...
fn unique_name() -> String {
    ...
    format!("nasm_{:?}_{}", tid, id).replace(['(', ')', ' '], "_")
}
...
fn parse_listing(listing: &str) -> Vec<(String, Vec<u8>)> {
    ...
    for line in listing.lines() {
        ...
        let is_continuation =
            source.is_empty() && prev_line_num == Some(line_num) && !results.is_empty();
...
pub const REGS8: &[(Reg, &str)] = &[(AL, "al"), (CL, "cl"), (DL, "dl"), (BL, "bl")];
...
pub const REGS8_HIGH: &[(Reg, &str)] = &[(AH, "ah"), (CH, "ch"), (DH, "dh"), (BH, "bh")];

pub const REGS8_SPL: &[(Reg, &str)] = &[(SPL, "spl"), (BPL, "bpl"), (SIL, "sil"), (DIL, "dil")];
...
fn mems_sized(make_addr: fn(RegExp) -> Address, nasm_prefix: &str) -> Vec<(Address, String)> {
    addr_exprs()
        .into_iter()
        .map(|(exp, bracket)| (make_addr(exp), format!("{} {}", nasm_prefix, bracket)))
        .collect()
}

pub fn mems8() -> Vec<(Address, String)> {
    mems_sized(byte_ptr, "byte")
}
pub fn mems16() -> Vec<(Address, String)> {
    mems_sized(word_ptr, "word")
}
pub fn mems32() -> Vec<(Address, String)> {
    mems_sized(dword_ptr, "dword")
}
pub fn mems64() -> Vec<(Address, String)> {
    mems_sized(qword_ptr, "qword")
}
pub fn mems128() -> Vec<(Address, String)> {
    mems_sized(xmmword_ptr, "oword")
}
pub fn mems256() -> Vec<(Address, String)> {
    mems_sized(ymmword_ptr, "yword")
}
pub fn mems512() -> Vec<(Address, String)> {
    mems_sized(zmmword_ptr, "zword")
}
...
pub fn normalize_prefix(bytes: &[u8]) -> Vec<u8> {
    ...
    let prefix_end = bytes
        .iter()
        .position(|b| !LEGACY_PREFIXES.contains(b))
        .unwrap_or(bytes.len());

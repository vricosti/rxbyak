...
fn test_nm_evex_arith() {
    ...
    for (addr, nasm_mem) in mems512() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vaddps(ZMM0, ZMM1, addr)),
        ));
    }
...
fn test_nm_evex_extended_regs() {
    ...
    for &(d, dn, s1, s1n, s2, s2n) in combos {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmulpd(d, s1, s2)),
        ));
    }

    // Extended XMM registers (xmm16-31)
    insns.push((
        "vaddps xmm16, xmm17, xmm18".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(XMM16, XMM17, XMM18)),
    ));
    insns.push((
        "vaddps xmm31, xmm0, xmm16".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(XMM31, XMM0, XMM16)),
    ));
    // Extended YMM registers
    insns.push((
        "vaddps ymm16, ymm17, ymm18".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(YMM16, YMM17, YMM18)),
    ));
...
fn test_nm_evex_opmask() {
    ...
    for &(k, ki, kn) in &[(K1, 1u8, "k1"), (K3, 3, "k3"), (K7, 7, "k7")] {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vaddps(ZMM0.k(ki), ZMM1, ZMM2)),
        ));
    }

    // vaddpd xmm{k2}, xmm, xmm
    insns.push((
        "vaddpd xmm0{k2}, xmm1, xmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddpd(XMM0.k(2), XMM1, XMM2)),
    ));
    // vaddps ymm{k5}, ymm, ymm
    insns.push((
        "vaddps ymm0{k5}, ymm1, ymm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(YMM0.k(5), YMM1, YMM2)),
    ));

    // vmovdqa32 with mask
    insns.push((
        "vmovdqa32 zmm0{k1}, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(ZMM0.k(1), ZMM1)),
    ));
    insns.push((
        "vmovdqa32 zmm16{k7}, zmm31".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(ZMM16.k(7), ZMM31)),
    ));
...
fn test_nm_evex_zeroing() {
    ...
    insns.push((
        "vaddps zmm0{k1}{z}, zmm1, zmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(ZMM0.k(1).z(), ZMM1, ZMM2)),
    ));
    insns.push((
        "vaddps zmm16{k7}{z}, zmm17, zmm31".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddps(ZMM16.k(7).z(), ZMM17, ZMM31)),
    ));
    // vaddpd xmm{k2}{z}, xmm, xmm
    insns.push((
        "vaddpd xmm0{k2}{z}, xmm1, xmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddpd(XMM0.k(2).z(), XMM1, XMM2)),
    ));
    // vmovdqa32 with mask + zeroing
    insns.push((
        "vmovdqa32 zmm0{k1}{z}, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(ZMM0.k(1).z(), ZMM1)),
    ));
...
fn test_nm_evex_rounding() {
    ...
    for &(rnd, nasm_rnd) in modes {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vaddps(ZMM0, ZMM1, ZMM2.rounding(rnd))),
        ));
    }

    // vaddpd with rounding
    insns.push((
        "vaddpd zmm0, zmm1, zmm2, {rn-sae}".into(),
        Box::new(|a: &mut CodeAssembler| a.vaddpd(ZMM0, ZMM1, ZMM2.rounding(Rounding::RnSae))),
    ));

    // Combined: mask + zeroing + rounding
    insns.push((
        "vaddps zmm0{k1}{z}, zmm1, zmm2, {rn-sae}".into(),
        Box::new(|a: &mut CodeAssembler| {
            a.vaddps(ZMM0.k(1).z(), ZMM1, ZMM2.rounding(Rounding::RnSae))
        }),
    ));
...
fn test_nm_evex_mov() {
    ...
    // vmovdqa32 zmm, zmm
    insns.push((
        "vmovdqa32 zmm0, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(ZMM0, ZMM1)),
    ));
    insns.push((
        "vmovdqa32 zmm16, zmm31".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(ZMM16, ZMM31)),
    ));
    // vmovdqa64 zmm, zmm
    insns.push((
        "vmovdqa64 zmm0, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa64(ZMM0, ZMM1)),
    ));
    // vmovdqu32 zmm, zmm
    insns.push((
        "vmovdqu32 zmm0, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqu32(ZMM0, ZMM1)),
    ));
    // vmovdqu64 zmm, zmm
    insns.push((
        "vmovdqu64 zmm0, zmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqu64(ZMM0, ZMM1)),
    ));
    ...
    for (addr, nasm_mem) in mems512() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovdqa32(ZMM0, addr)),
        ));
    }
    // vmovdqa32 [mem], zmm
    for (addr, nasm_mem) in mems512() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovdqa32(addr, ZMM0)),
        ));
    }

    // xmm/ymm EVEX mov
    insns.push((
        "vmovdqa32 xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(XMM0, XMM1)),
    ));
    insns.push((
        "vmovdqa32 ymm0, ymm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovdqa32(YMM0, YMM1)),
    ));
...
fn test_nm_opmask_mov() {
    ...
    insns.push((
        "kmovw k1, k2".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovw(K1, K2)),
    ));
    insns.push((
        "kmovw k7, k0".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovw(K7, K0)),
    ));
    // kmovw k, r32
    insns.push((
        "kmovw k1, eax".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovw(K1, EAX)),
    ));
    insns.push((
        "kmovw k7, r8d".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovw(K7, R8D)),
    ));
    // kmovw r32, k
    insns.push((
        "kmovw eax, k1".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovw(EAX, K1)),
    ));
    // kmovb
    insns.push((
        "kmovb k1, k2".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovb(K1, K2)),
    ));
    // kmovd
    insns.push((
        "kmovd k1, k2".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovd(K1, K2)),
    ));
    insns.push((
        "kmovd k1, eax".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovd(K1, EAX)),
    ));
    // kmovq
    insns.push((
        "kmovq k1, k2".into(),
        Box::new(|a: &mut CodeAssembler| a.kmovq(K1, K2)),
    ));
    ...
    for (addr, nasm_mem) in mems_nosizeptr() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.kmovw(K1, addr)),
        ));
    }
...
fn test_nm_opmask_logic() {
    ...
    for &(name, op_fn) in ops3 {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| op_fn(a, K1, K2, K3)),
        ));
        let asm = format!("{} k7, k0, k6", name);
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| op_fn(a, K7, K0, K6)),
        ));
    }
    ...
    // kshiftlw k, k, imm8
    insns.push((
        "kshiftlw k1, k2, 4".into(),
        Box::new(|a: &mut CodeAssembler| a.kshiftlw(K1, K2, 4)),
    ));
    insns.push((
        "kshiftrw k1, k2, 8".into(),
        Box::new(|a: &mut CodeAssembler| a.kshiftrw(K1, K2, 8)),
    ));

    // kunpckbw k, k, k
    insns.push((
        "kunpckbw k1, k2, k3".into(),
        Box::new(|a: &mut CodeAssembler| a.kunpckbw(K1, K2, K3)),
    ));
...
fn test_nm_evex_extract() {
    ...
    insns.push((
        "vextractf32x4 xmm0, zmm1, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractf32x4(XMM0, ZMM1, 1)),
    ));
    insns.push((
        "vextractf32x4 xmm16, zmm31, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractf32x4(XMM16, ZMM31, 3)),
    ));
    // vextracti32x4
    insns.push((
        "vextracti32x4 xmm0, zmm1, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.vextracti32x4(XMM0, ZMM1, 2)),
    ));
    // vextractf64x4 ymm, zmm, imm8
    insns.push((
        "vextractf64x4 ymm0, zmm1, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractf64x4(YMM0, ZMM1, 1)),
    ));
    // vextracti64x4
    insns.push((
        "vextracti64x4 ymm0, zmm1, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vextracti64x4(YMM0, ZMM1, 1)),
    ));

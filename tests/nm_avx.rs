...
fn test_nm_avx_arith_xmm() {
    ...
    for &(name, op_fn) in ops {
        for &(d, dn, s1, s1n, s2, s2n) in triples {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| op_fn(a, d, s1, s2)),
            ));
        }
    }
...
fn test_nm_avx_arith_mem() {
    ...
    for (addr, nasm_mem) in mems128() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vaddps(XMM0, XMM1, addr)),
        ));
    }
    // vaddps ymm, ymm, [mem]
    for (addr, nasm_mem) in mems256() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vaddps(YMM0, YMM1, addr)),
        ));
    }
...
fn test_nm_avx_mov() {
    ...
    let xpairs: &[(Reg, &str, Reg, &str)] =
        &[(XMM0, "xmm0", XMM1, "xmm1"), (XMM8, "xmm8", XMM15, "xmm15")];
    let ypairs: &[(Reg, &str, Reg, &str)] =
        &[(YMM0, "ymm0", YMM1, "ymm1"), (YMM8, "ymm8", YMM15, "ymm15")];
    ...
    for &(name, op_fn) in load_ops {
        for (addr, nasm_mem) in mems128() {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| op_fn(a, XMM0, addr)),
            ));
        }
        for (addr, nasm_mem) in mems256() {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| op_fn(a, YMM0, addr)),
            ));
        }
    }
    ...
    for &(name, op_fn) in store_ops {
        for (addr, nasm_mem) in mems128() {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| op_fn(a, addr, XMM0)),
            ));
        }
    }
...
fn test_nm_avx_int() {
    ...
    for (addr, nasm_mem) in mems128() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vpaddd(XMM0, XMM1, addr)),
        ));
    }
...
fn test_nm_avx_shuffle() {
    ...
    insns.push((
        "vpshufd xmm0, xmm1, 0x1b".into(),
        Box::new(|a: &mut CodeAssembler| a.vpshufd(XMM0, XMM1, 0x1B)),
    ));
    insns.push((
        "vpshufd xmm8, xmm15, 0xff".into(),
        Box::new(|a: &mut CodeAssembler| a.vpshufd(XMM8, XMM15, 0xFF)),
    ));
    insns.push((
        "vpshufd ymm0, ymm1, 0xe4".into(),
        Box::new(|a: &mut CodeAssembler| a.vpshufd(YMM0, YMM1, 0xE4)),
    ));

    // vshufps xmm, xmm, xmm, imm8 (3-op)
    insns.push((
        "vshufps xmm0, xmm1, xmm2, 0xe4".into(),
        Box::new(|a: &mut CodeAssembler| a.vshufps(XMM0, XMM1, XMM2, 0xE4)),
    ));
    insns.push((
        "vshufps ymm0, ymm1, ymm2, 0x1b".into(),
        Box::new(|a: &mut CodeAssembler| a.vshufps(YMM0, YMM1, YMM2, 0x1B)),
    ));
    insns.push((
        "vshufps xmm8, xmm9, xmm15, 0x00".into(),
        Box::new(|a: &mut CodeAssembler| a.vshufps(XMM8, XMM9, XMM15, 0x00)),
    ));

    // vshufpd
    insns.push((
        "vshufpd xmm0, xmm1, xmm2, 0x01".into(),
        Box::new(|a: &mut CodeAssembler| a.vshufpd(XMM0, XMM1, XMM2, 0x01)),
    ));

    // vpshufb xmm, xmm, xmm (3-op)
    insns.push((
        "vpshufb xmm0, xmm1, xmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vpshufb(XMM0, XMM1, XMM2)),
    ));
    insns.push((
        "vpshufb xmm8, xmm9, xmm15".into(),
        Box::new(|a: &mut CodeAssembler| a.vpshufb(XMM8, XMM9, XMM15)),
    ));

    // vpalignr xmm, xmm, xmm, imm8 (3-op)
    insns.push((
        "vpalignr xmm0, xmm1, xmm2, 4".into(),
        Box::new(|a: &mut CodeAssembler| a.vpalignr(XMM0, XMM1, XMM2, 4)),
    ));
    insns.push((
        "vpalignr xmm8, xmm9, xmm15, 8".into(),
        Box::new(|a: &mut CodeAssembler| a.vpalignr(XMM8, XMM9, XMM15, 8)),
    ));
...
fn test_nm_avx_extract() {
    ...
    insns.push((
        "vextractf128 xmm0, ymm1, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractf128(XMM0, YMM1, 1)),
    ));
    insns.push((
        "vextractf128 xmm8, ymm15, 0".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractf128(XMM8, YMM15, 0)),
    ));
    // vextracti128 xmm, ymm, imm8
    insns.push((
        "vextracti128 xmm0, ymm1, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vextracti128(XMM0, YMM1, 1)),
    ));

    // vpextrb r32, xmm, imm8
    insns.push((
        "vpextrb eax, xmm0, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrb(EAX, XMM0, 3)),
    ));
    insns.push((
        "vpextrb r8d, xmm8, 5".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrb(R8D, XMM8, 5)),
    ));
    // vpextrw r32, xmm, imm8
    insns.push((
        "vpextrw eax, xmm0, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrw(EAX, XMM0, 3)),
    ));
    // vpextrd r32, xmm, imm8
    insns.push((
        "vpextrd eax, xmm0, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrd(EAX, XMM0, 2)),
    ));
    insns.push((
        "vpextrd r8d, xmm8, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrd(R8D, XMM8, 1)),
    ));
    // vpextrq r64, xmm, imm8
    insns.push((
        "vpextrq rax, xmm0, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vpextrq(RAX, XMM0, 1)),
    ));
    // vextractps r32, xmm, imm8
    insns.push((
        "vextractps eax, xmm0, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.vextractps(EAX, XMM0, 2)),
    ));

    // vpinsrb/w/d/q
    insns.push((
        "vpinsrb xmm0, xmm1, eax, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.vpinsrb(XMM0, XMM1, EAX, 3)),
    ));
    insns.push((
        "vpinsrw xmm0, xmm1, eax, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.vpinsrw(XMM0, XMM1, EAX, 3)),
    ));
    insns.push((
        "vpinsrd xmm0, xmm1, eax, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.vpinsrd(XMM0, XMM1, EAX, 2)),
    ));
    insns.push((
        "vpinsrq xmm0, xmm1, rax, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vpinsrq(XMM0, XMM1, RAX, 1)),
    ));
    // vinsertps
    insns.push((
        "vinsertps xmm0, xmm1, xmm2, 0x10".into(),
        Box::new(|a: &mut CodeAssembler| a.vinsertps(XMM0, XMM1, XMM2, 0x10)),
    ));
    // vinsertf128/i128
    insns.push((
        "vinsertf128 ymm0, ymm1, xmm2, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vinsertf128(YMM0, YMM1, XMM2, 1)),
    ));
    insns.push((
        "vinserti128 ymm0, ymm1, xmm2, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.vinserti128(YMM0, YMM1, XMM2, 1)),
    ));
...
fn test_nm_avx_broadcast() {
    ...
    insns.push((
        "vbroadcastss xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vbroadcastss(XMM0, XMM1)),
    ));
    insns.push((
        "vbroadcastss ymm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vbroadcastss(YMM0, XMM1)),
    ));
    insns.push((
        "vbroadcastss xmm8, xmm15".into(),
        Box::new(|a: &mut CodeAssembler| a.vbroadcastss(XMM8, XMM15)),
    ));
    ...
    for (addr, nasm_mem) in mems32() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vbroadcastss(XMM0, addr)),
        ));
    }
    // vbroadcastsd ymm, xmm (AVX2)
    insns.push((
        "vbroadcastsd ymm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vbroadcastsd(YMM0, XMM1)),
    ));
...
fn test_nm_avx_misc() {
    ...
    insns.push((
        "vzeroall".into(),
        Box::new(|a: &mut CodeAssembler| a.vzeroall()),
    ));
    insns.push((
        "vzeroupper".into(),
        Box::new(|a: &mut CodeAssembler| a.vzeroupper()),
    ));

    // vmovmskps r32, xmm/ymm
    insns.push((
        "vmovmskps eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovmskps(EAX, XMM0)),
    ));
    insns.push((
        "vmovmskps eax, ymm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovmskps(EAX, YMM0)),
    ));
    insns.push((
        "vmovmskps r8d, xmm8".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovmskps(R8D, XMM8)),
    ));
    // vmovmskpd
    insns.push((
        "vmovmskpd eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovmskpd(EAX, XMM0)),
    ));
    insns.push((
        "vmovmskpd eax, ymm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovmskpd(EAX, YMM0)),
    ));
    // vpmovmskb
    insns.push((
        "vpmovmskb eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vpmovmskb(EAX, XMM0)),
    ));
    insns.push((
        "vpmovmskb eax, ymm0".into(),
        Box::new(|a: &mut CodeAssembler| a.vpmovmskb(EAX, YMM0)),
    ));
    ...
    for (addr, nasm_mem) in mems128() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovntdq(addr, XMM0)),
        ));
    }

    // vsqrtps xmm, xmm
    insns.push((
        "vsqrtps xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vsqrtps(XMM0, XMM1)),
    ));
    insns.push((
        "vsqrtps ymm0, ymm1".into(),
        Box::new(|a: &mut CodeAssembler| a.vsqrtps(YMM0, YMM1)),
    ));
...
fn test_nm_avx_special() {
    ...
    insns.push((
        "vmovss xmm0, xmm1, xmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovss(XMM0, XMM1, Some(XMM2))),
    ));
    insns.push((
        "vmovss xmm8, xmm9, xmm15".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovss(XMM8, XMM9, Some(XMM15))),
    ));
    // vmovsd xmm, xmm, xmm (3-operand)
    insns.push((
        "vmovsd xmm0, xmm1, xmm2".into(),
        Box::new(|a: &mut CodeAssembler| a.vmovsd(XMM0, XMM1, Some(XMM2))),
    ));
    ...
    for (addr, nasm_mem) in mems32() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovss(XMM0, addr, None)),
        ));
    }
    ...
    for (addr, nasm_mem) in mems32() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovss(addr, XMM0, None)),
        ));
    }
    // vmovsd xmm, [mem] (load)
    for (addr, nasm_mem) in mems64() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovsd(XMM0, addr, None)),
        ));
    }
    ...
    for (addr, nasm_mem) in mems64() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.vmovlpd_store(addr, XMM0)),
        ));
    }

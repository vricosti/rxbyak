...
fn test_nm_sse_movd_movq() {
    ...
    insns.push((
        "movq xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.movq(XMM0, XMM1)),
    ));
    insns.push((
        "movq xmm8, xmm15".into(),
        Box::new(|a: &mut CodeAssembler| a.movq(XMM8, XMM15)),
    ));
...
fn test_nm_sse_movss_movsd() {
    ...
    for &(dst, dn, src, sn) in pairs {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movss(dst, src)),
        ));
        let asm = format!("movsd {}, {}", dn, sn);
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movsd(dst, src)),
        ));
    }
    ...
    for (addr, nasm_mem) in mems32() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movss(XMM0, addr)),
        ));
    }
    // movss [mem], xmm (store)
    for (addr, nasm_mem) in mems32() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movss(addr, XMM0)),
        ));
    }
    // movsd xmm, [mem] (load)
    for (addr, nasm_mem) in mems64() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movsd(XMM0, addr)),
        ));
    }
    // movsd [mem], xmm (store)
    for (addr, nasm_mem) in mems64() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movsd(addr, XMM0)),
        ));
    }
...
fn test_nm_sse_movhlps() {
    ...
    for (addr, nasm_mem) in mems64() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movlpd_store(addr, XMM0)),
        ));
    }
...
fn test_nm_sse_shuffle() {
    ...
    for &(dst, dn, src, sn) in pairs {
        for imm in [0x00u8, 0x1B, 0xFF] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.pshufd(dst, src, imm)),
            ));
        }
    }
    // shufps xmm, xmm, imm8
    for &(dst, dn, src, sn) in pairs {
        for imm in [0x00u8, 0xE4] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.shufps(dst, src, imm)),
            ));
        }
    }
    // shufpd xmm, xmm, imm8
    for &(dst, dn, src, sn) in pairs {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.shufpd(dst, src, 0x01)),
        ));
    }
    // pshufb xmm, xmm
    for &(dst, dn, src, sn) in pairs {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.pshufb(dst, src)),
        ));
    }
    ...
    for &(dst, dn, src, sn) in pairs {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.palignr(dst, src, 4)),
        ));
    }
...
fn test_nm_sse_cvt() {
    ...
    for &(xmm, xn) in &[(XMM0, "xmm0"), (XMM8, "xmm8"), (XMM14, "xmm14")] {
        for &(gpr, gn) in &[(EAX, "eax"), (R8D, "r8d")] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.cvtsi2ss(xmm, gpr)),
            ));
        }
        for &(gpr, gn) in &[(RAX, "rax"), (R8, "r8")] {
            let asm = format!("cvtsi2ss {}, {}", xn, gn);
    ...
    for &(xmm, xn) in &[(XMM0, "xmm0"), (XMM8, "xmm8")] {
        for &(gpr, gn) in &[(EAX, "eax"), (RAX, "rax")] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.cvtsi2sd(xmm, gpr)),
            ));
        }
    }
    // cvtss2sd / cvtsd2ss
    insns.push((
        "cvtss2sd xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.cvtss2sd(XMM0, XMM1)),
    ));
    insns.push((
        "cvtsd2ss xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.cvtsd2ss(XMM0, XMM1)),
    ));
    insns.push((
        "cvtss2sd xmm8, xmm15".into(),
        Box::new(|a: &mut CodeAssembler| a.cvtss2sd(XMM8, XMM15)),
    ));

    // cvtps2dq / cvtdq2ps / cvttps2dq
    insns.push((
        "cvtps2dq xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.cvtps2dq(XMM0, XMM1)),
    ));
    insns.push((
        "cvtdq2ps xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.cvtdq2ps(XMM0, XMM1)),
    ));
    insns.push((
        "cvttps2dq xmm0, xmm1".into(),
        Box::new(|a: &mut CodeAssembler| a.cvttps2dq(XMM0, XMM1)),
    ));

    // cvtss2si / cvtsd2si / cvttss2si / cvttsd2si
    for &(gpr, gn) in &[(EAX, "eax"), (RAX, "rax")] {
        insns.push((
            format!("cvtss2si {}, xmm0", gn),
            Box::new(move |a: &mut CodeAssembler| a.cvtss2si(gpr, XMM0)),
        ));
        insns.push((
            format!("cvtsd2si {}, xmm0", gn),
            Box::new(move |a: &mut CodeAssembler| a.cvtsd2si(gpr, XMM0)),
        ));
        insns.push((
            format!("cvttss2si {}, xmm0", gn),
            Box::new(move |a: &mut CodeAssembler| a.cvttss2si(gpr, XMM0)),
        ));
        insns.push((
            format!("cvttsd2si {}, xmm0", gn),
            Box::new(move |a: &mut CodeAssembler| a.cvttsd2si(gpr, XMM0)),
        ));
    }
...
fn test_nm_sse_extract_insert() {
    ...
    for &(gpr, gn) in &[(EAX, "eax"), (R8D, "r8d")] {
        for &(xmm, xn) in &[(XMM0, "xmm0"), (XMM8, "xmm8")] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.pextrb(gpr, xmm, 3)),
            ));
        }
    }
    // pextrw r32, xmm, imm8
    for &(gpr, gn) in &[(EAX, "eax"), (R8D, "r8d")] {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.pextrw(gpr, XMM0, 3)),
        ));
    }
    // pextrd r32, xmm, imm8
    insns.push((
        "pextrd eax, xmm0, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.pextrd(EAX, XMM0, 2)),
    ));
    insns.push((
        "pextrd r8d, xmm8, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.pextrd(R8D, XMM8, 1)),
    ));
    // pextrq r64, xmm, imm8
    insns.push((
        "pextrq rax, xmm0, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.pextrq(RAX, XMM0, 1)),
    ));
    insns.push((
        "pextrq r8, xmm8, 0".into(),
        Box::new(|a: &mut CodeAssembler| a.pextrq(R8, XMM8, 0)),
    ));
    // extractps r32, xmm, imm8
    insns.push((
        "extractps eax, xmm0, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.extractps(EAX, XMM0, 2)),
    ));

    // pinsrb xmm, r32, imm8
    insns.push((
        "pinsrb xmm0, eax, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrb(XMM0, EAX, 3)),
    ));
    insns.push((
        "pinsrb xmm8, r8d, 5".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrb(XMM8, R8D, 5)),
    ));
    // pinsrw xmm, r32, imm8
    insns.push((
        "pinsrw xmm0, eax, 3".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrw(XMM0, EAX, 3)),
    ));
    // pinsrd xmm, r32, imm8
    insns.push((
        "pinsrd xmm0, eax, 2".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrd(XMM0, EAX, 2)),
    ));
    insns.push((
        "pinsrd xmm8, r8d, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrd(XMM8, R8D, 1)),
    ));
    // pinsrq xmm, r64, imm8
    insns.push((
        "pinsrq xmm0, rax, 1".into(),
        Box::new(|a: &mut CodeAssembler| a.pinsrq(XMM0, RAX, 1)),
    ));
...
fn test_nm_sse_misc() {
    ...
    // movmskps r32, xmm
    insns.push((
        "movmskps eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.movmskps(EAX, XMM0)),
    ));
    insns.push((
        "movmskps eax, xmm8".into(),
        Box::new(|a: &mut CodeAssembler| a.movmskps(EAX, XMM8)),
    ));
    insns.push((
        "movmskps r8d, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.movmskps(R8D, XMM0)),
    ));
    // movmskpd r32, xmm
    insns.push((
        "movmskpd eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.movmskpd(EAX, XMM0)),
    ));
    insns.push((
        "movmskpd eax, xmm8".into(),
        Box::new(|a: &mut CodeAssembler| a.movmskpd(EAX, XMM8)),
    ));
    // pmovmskb r32, xmm
    insns.push((
        "pmovmskb eax, xmm0".into(),
        Box::new(|a: &mut CodeAssembler| a.pmovmskb(EAX, XMM0)),
    ));
    insns.push((
        "pmovmskb eax, xmm8".into(),
        Box::new(|a: &mut CodeAssembler| a.pmovmskb(EAX, XMM8)),
    ));
    ...
    for (addr, nasm_mem) in mems128() {
        ...
        insns.push((
            asm,
            Box::new(move |a: &mut CodeAssembler| a.movntdq(addr, XMM0)),
        ));
    }
...
fn test_nm_sse_pslldq_psrldq() {
    ...
    for &(xmm, xn) in &[
        ...
        for imm in [0u8, 1, 8, 15] {
            ...
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.pslldq(xmm, imm)),
            ));
            let asm = format!("psrldq {}, {}", xn, imm);
            insns.push((
                asm,
                Box::new(move |a: &mut CodeAssembler| a.psrldq(xmm, imm)),
            ));
        }
    }

...
fn test_nm_amx_basic() {
    ...
    for &(reg, name) in tmms {
        ...
        cases.push((
            nasm_str,
            Box::new(move |a: &mut CodeAssembler| a.tilezero(reg)),
        ));
    }
...
fn test_nm_amx_tdp() {
    ...
    for &(nasm_name, method) in tdp_ops {
        for &(dst, src1, src2, dn, s1n, s2n) in tmm_combos {
            ...
            cases.push((
                nasm_str,
                Box::new(move |a: &mut CodeAssembler| method(a, dst, src1, src2)),
            ));
        }
    }
...
fn test_nm_blendv() {
    ...
    for &(dst, src, dn, sn) in xmm_pairs {
        ...
        cases.push((
            nasm_str,
            Box::new(move |a: &mut CodeAssembler| a.blendvps(dst, src)),
        ));
    }
    for &(dst, src, dn, sn) in xmm_pairs {
        ...
        cases.push((
            nasm_str,
            Box::new(move |a: &mut CodeAssembler| a.blendvpd(dst, src)),
        ));
    }
    for &(dst, src, dn, sn) in xmm_pairs {
        ...
        cases.push((
            nasm_str,
            Box::new(move |a: &mut CodeAssembler| a.pblendvb(dst, src)),
        ));
    }
...
fn test_nm_vcvtps2ph() {
    ...
    for &(dst, src, dn, sn) in pairs {
        ...
        cases.push((
            nasm_str,
            Box::new(move |a: &mut CodeAssembler| a.vcvtps2ph(dst, src, 0x04)),
        ));
    }

    // vcvtps2ph xmm, ymm, imm8
    let ymm_pairs: &[(Reg, Reg, &str, &str)] =
        &[(XMM0, YMM1, "xmm0", "ymm1"), (XMM5, YMM10, "xmm5", "ymm10")];
